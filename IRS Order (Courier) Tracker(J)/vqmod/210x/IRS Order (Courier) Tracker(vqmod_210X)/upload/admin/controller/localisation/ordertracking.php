...
class ControllerLocalisationOrdertracking extends Controller {
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {    //print_r($this->request->post);exit;
			...
			$this->response->redirect($this->url->link('localisation/ordertracking', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() { 
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/ordertracking', 'token=' . $this->session->data['token'] . $url, 'SSL'),           
		);

		$data['add'] = $this->url->link('localisation/ordertracking/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('localisation/ordertracking/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$data['sort_name'] = $this->url->link('localisation/ordertracking', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_code'] = $this->url->link('localisation/ordertracking', 'token=' . $this->session->data['token'] . '&sort=code' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('localisation/ordertracking', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('localisation/ordertracking', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() { 
		...
		if (!isset($this->request->get['ordertracking_id'])) { 
			$data['action'] = $this->url->link('localisation/ordertracking/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {  
			$data['action'] = $this->url->link('localisation/ordertracking/edit', 'token=' . $this->session->data['token'] . '&ordertracking_id=' . $this->request->get['ordertracking_id'] . $url, 'SSL');//echo 123;exit;
		}

		$data['cancel'] = $this->url->link('localisation/ordertracking', 'token=' . $this->session->data['token'] . $url, 'SSL');
