...
class ControllerSaleBirthdaycalender extends Controller {
	...
	public function index()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if ($this->request->post['id']=="") {
                $this->model_sale_birthdaycalender->addbirthdaysetting($this->request->post);
            } else {
                $this->model_sale_birthdaycalender->editbirthdaysetting($this->request->post);
            }
            $this->session->data['success'] = $this->language->get('text_success');
            $this->response->redirect($this->url->link('sale/birthdaycalender', 'token=' . $this->session->data['token'], 'SSL'));
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('sale/birthdaycalender', 'token=' . $this->session->data['token'] , 'SSL')
        );
        ...
        $product_info = $this->model_sale_birthdaycalender->getbirthdaysetting();
    ...
    public function getBirthdaydate() {
       ...
       foreach($data1 as $key => $totaldata ){
            $datevalue=date("Y")."-".date("m-d",strtotime($totaldata["Dateofbirth"]));
            $jsonvalue[]=array(
