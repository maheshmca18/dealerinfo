...
class ModelLocalisationDeliverytimeslot extends Model {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "deliverydatetime SET from_time = '" . $this->db->escape($data['from_time']) . "',to_time = '" . $this->db->escape($data['to_time']) . "', status = '" . $this->db->escape($data['status']) . "'");

	}

	public function editTimeslot($deliverytime_id, $data) { //print_r($data);exit;
      
		$this->db->query("UPDATE " . DB_PREFIX . "deliverydatetime SET from_time = '" . $this->db->escape($data['from_time']) . "',to_time = '" . $this->db->escape($data['to_time']) . "', status = '" . $this->db->escape($data['status']) . "'  WHERE deliverytime_id = '" . (int)$deliverytime_id . "'");
