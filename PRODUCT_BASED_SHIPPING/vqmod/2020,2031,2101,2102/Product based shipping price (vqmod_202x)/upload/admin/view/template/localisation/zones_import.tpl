...
<div id="content">
	...
	<div class="content">
			...
			$customer_table_data .="<div class='table-responsive'><table class='table table-bordered table-hover'><thead><tr><td class='text-center'>Zone Location</td><td class='text-left'>Zone Postcode</td></tr></thead><tbody>";						
			...
			foreach($sampletabledata as $zonedata)
				{		
								$customer_table_data .="<tr><td class='left'>".$zonedata['zone_location']."</td><td class='left'>".$zonedata['postcode']."</td></tr>";


			if($excel_field_validate) {
				if ($zonedata['zone_location'] == '') { 
					$excel_fields_error['zone_location']="Enter Zone Location";
					$excel_field_validate=0;
				}

				if ($zonedata['postcode'] == '') { 
					$excel_fields_error['postcode']="Enter Postcode";
					$excel_field_validate=0;
				}

foreach($getzonename_validations as $getzonename_validation) {

if ($getzonename_validation['zone_location'] == $zonedata['zone_location']) { 
					$excel_fields_error['zone_location_exist']=$zonedata['zone_location']. " location is already exist";
					$excel_field_validate=0;
				 }
}
...
if ($getzonename_validation['postcode'] == $zonedata['postcode']) { 
					$excel_fields_error['postcode_exist']=$zonedata['postcode']. " postcode is already exist";
					$excel_field_validate=0;
				 }
}
