<?php
class ControllerExtensionModuleRecaptchaprice extends Controller {
	private $error = array(); 

	public function index() {   
		$this->language->load('extension/module/recaptchaprice');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/recaptchaprice', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/module/recaptchaprice', 'token=' . $this->session->data['token'], true);
		...
		$this->template = 'extension/module/recaptchaprice.tpl';
		//$this->children = array(
		//	'common/header',
		//	'common/footer',
		//);
		...
		$this->response->setOutput($this->load->view('extension/module/recaptchaprice.tpl', $data));
	}

	private function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/recaptchaprice')) {
	...
	public function uninstall() {
		$this->load->model('setting/setting');
		$this->model_setting_setting->deleteSetting('recaptcha');
	}
}