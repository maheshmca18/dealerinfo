    ...
    $(document).ready(function() { //READY STARTS HERE
        ...
        //--------------------ADD onclick function Start---------------------------------//
                ...
                if ($category == "") {
                    ...
                    $.ajax({
                        url: "index.php?route=module/advancefilter/autosearch&token=<?php echo $token; ?>",
                        type: "POST",
                        dataType: "json",
                    ...
                    $.ajax({
                        url: "index.php?route=module/advancefilter/add&token=<?php echo $token; ?>",
                        type: "POST",
                        dataType: "json",
        ...
        $(document).on('click','.delete',function(){
            ...
            $.ajax({
                type: "POST",
                url: "index.php?route=module/advancefilter/delete&token=<?php echo $token; ?>",
                data: {id: $id},
                dataType: "json",
                success: function (data) {
                    if (data != "") {
        ...
        $(document).on('click','.edit',function(){
           $id =  $(this).attr('adf_id');
            $.ajax({
                type: "POST",
                url: "index.php?route=module/advancefilter/edit&token=<?php echo $token; ?>",
                data: {id: $id},
                dataType: "json",
                success: function (data) {
                    if (data != "") {
                        ...
                        html +=" <tr>";
                          $.each(data, function (id, row) {
                            ...
                            // option start here
                              ...
                              html += " <?php } ?><span class='errormsgoption' style='font-size: 1em; color: #681818;'></span></div></div></td>";
                                //attribute end here
                              html += " <td><div > <button title='Save' style='margin-left: 19px;margin-top: 75px;' edit_id='"+row['af_id']+"' class='addupdate btn btn-primary'><i class='fa fa-save'></i></button><button title='cancel' adf_id='" + row['af_id'] + "' class='editdelete btn btn-danger' style='margin-left: 10px; margin-top: 73px;'><i class='fa fa-reply'></i></button></div></td>";
                              html += "</div>";
                            //show in table
                        });//for each
                        html += "</tr></table></div>";
        ...
        $(document).on('click','.editdelete',function(){
            ...
            $.ajax({
                type: "POST",
                url: "index.php?route=module/advancefilter/editcancel&token=<?php echo $token; ?>",
                data: {id: $id},
                dataType: "json",
                success: function (data) {
                    if (data != "") {
        ...
        $(document).on('click','.addupdate',function(){
            ...
            $attri=attribute.join(",");
             if($optiondetails=="" && $attri=="" ) {
                ...
                $.ajax({
                    url: "index.php?route=module/advancefilter/update&token=<?php echo $token; ?>",
                    type: "POST",
                    dataType: "json",
