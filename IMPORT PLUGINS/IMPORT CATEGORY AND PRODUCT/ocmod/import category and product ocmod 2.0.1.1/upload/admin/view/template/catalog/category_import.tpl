...
<div id="content">
	...
	<div class="container-fluid">
					...
					foreach($sampletabledata as $categorydata)
						{		
							$category_table_data .="<tr><td class='text-left'>".$categorydata['category_description'][1]['name']."</td><td class='text-left'>".$categorydata['category_description'][1]['description']."</td><td class='text-left'>".$categorydata['parent']."</td></tr>";
							
							if($excel_field_validate)
							{
								if ($categorydata['category_description'][1]['name'] == '') { 
									$excel_fields_error['name']="Enter Category Name";
									$excel_field_validate=0;
									}
							}
							
							/*if(in_array($categorydata['category_description'][1]['name'],$category_names))
							{
									$excel_fields_error['category_name_exist']="Don't Repeat same Category";
									$excel_field_validate=0;
									continue;									
							}
							else
							{
									$category_names[] =	$categorydata['category_description'][1]['name'];							
							}*/

  if(in_array($categorydata['category_description'][1]['name'],$category_names) && in_array($categorydata['parent'],$category_names))
            {
