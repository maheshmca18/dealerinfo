...
$checkout = $wepay->request('/checkout/create', array(
 ...
 // "create_time"=> 1439582388,
  ...
  //"callback_uri"=> "http://ocdemo.indialocal.co.in/opc2031/",
...
$data['text_instruction_2'] = $this->language->get('text_instruction_2');
$data['button_confirm'] = $this->language->get('button_confirm');
//$data['redirect_wepay'] = $checkout->checkout_uri;
if(isset($error)){
//print_r($error);
$data['error']=	$error;
}else{
$data['redirect_wepay'] = $checkout->hosted_checkout->checkout_uri;
}



 if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/wepay.tpl')) {
            return $this->load->view($this->config->get('config_template') . '/template/payment/wepay.tpl', $data);
        } else {
            return $this->load->view('default/template/payment/wepay.tpl', $data);
        }
