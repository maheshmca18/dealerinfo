...
class ModelLocalisationPostcode extends Model {
	public function addPostcode($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "postcode_delivery SET status = '" . (int)$data['status'] . "', postcode = '" . $this->db->escape($data['postcode'])."', min_shipping = '" . $this->db->escape($data['min_shipping'])."'");
			
		$this->cache->delete('postcode');
	}
	
	public function editPostcode($postcode_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "postcode_delivery SET status = '" . (int)$data['status'] . "', postcode = '" . $this->db->escape($data['postcode']) . "', min_shipping = '" . $this->db->escape($data['min_shipping']) ."' WHERE postcode_id = '" . (int)$postcode_id. "'");

		$this->cache->delete('postcode');
	}
	
	public function deletePostcode($postcode_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "postcode_delivery WHERE postcode_id = '" . (int)$postcode_id . "'");
