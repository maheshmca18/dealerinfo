...
class ControllerExtensionModuleComboOfferm extends Controller {
	public function index($setting) {
		...
		if(isset($comboofferenable_status)) {
			...
			$this->load->model('extension/module/combo_offerm');
			...
			$modules = $this->model_extension_module_combo_offerm->getLayoutModules($layout_id, 'combo_offerm');
			...
			if (isset($this->request->get['product_id'])) {
				...
				$products = $this->model_extension_module_combo_offerm->getcomboofferproductbase($product_iddetails);
				...
				echo $category_id;
				$products = $this->model_extension_module_combo_offerm->getcombooffercategorybase($category_id);
			...
			if (isset($products)) {
				...
				foreach ($products as $combo_id) {
					$combo_info = $this->model_extension_module_combo_offerm->getComboofferdetails($combo_id);

					if (isset($combo_info['c_product'])) {
						$product_details = $this->model_extension_module_combo_offerm->getProduct($combo_info['c_product']);
			...
			if ($data['products']) {
				...
				return $this->load->view('extension/module/combo_offerm', $data);
			}
		}
	}
	...
	public function comboofferlist() {
		...
		if(isset($comboofferenable_status)) {
			...
			$data['products'] = array();
			$productsdetail = array();
			$combo_offerdetails = $this->model_extension_module_combo_offerm->getcombooffer();
...
$this->document->setTitle($product_info['meta_title']);
		...
		$this->response->setOutput($this->load->view('extension/module/combo_offer_list', $data));
		}
	}
	public function add() {
	...
	public function getoption($product_id){
		...
		if(isset($comboofferenable_status)) {
			...
			foreach ($product_id as $id) {
				...
				$product_options = $this->model_extension_module_combo_offerm->getProductOptions($product_info['product_id']);
	...
	return $this->load->view('extension/module/combo_offer_list_option', $data);
		}
	}
